...
- ในหน้า Dashboard กดปุ่ม **"Export JPG"** (สีเหลืองอำพัน มุมขวาบน) เพื่อบันทึกบอร์ดสรุปทั้งหมดเป็นไฟล์
  รูปภาพ `.jpg` ความละเอียดสูง (render ที่ pixel ratio 2x — คมชัดระดับจอ Retina โดยไม่เสี่ยงเกินขีดจำกัด
  ขนาด canvas ของเบราว์เซอร์บนมือถือ) ไม่แตก เหมาะสำหรับส่งเข้ากลุ่มไลน์หรือพิมพ์ติดบอร์ด
- ไฟล์จะถูกตั้งชื่ออัตโนมัติตามวันที่ เช่น `WH-Daily-Dashboard-23-7-2569.jpg`
- ก่อน capture ระบบจะรอให้ฟอนต์ (Google Fonts) โหลดเสร็จสมบูรณ์ก่อนเสมอ (`document.fonts.ready`) และ
  เลื่อนหน้าจอกลับไปบนสุดชั่วคราว เพื่อป้องกันปัญหาข้อความเคลื่อน/ไม่ตรงตำแหน่งที่อาจเกิดจากฟอนต์โหลดไม่ทัน
  หรือตำแหน่ง scroll ตอน capture
- ปุ่มนี้ใช้ไลบรารี [`html-to-image`](https://github.com/bubkoo/html-to-image) แปลงเฉพาะส่วนบอร์ด
  Dashboard (ไม่รวมแถบเมนู/ปุ่มต่างๆ) เป็นรูปภาพ ทำงานฝั่งเบราว์เซอร์ล้วน ไม่ต้องส่งข้อมูลขึ้นเซิร์ฟเวอร์
